...
async fn upload(
    ...
    while let Some(mut field) = multipart.next_field().await.map_err(bad_request)? {
        ...
        let temp_name = format!(".upload-{}-{}", std::process::id(), count);
        let temp_path = state.storage_root.join(&temp_name);
...
fn internal_error<E: std::fmt::Display>(error: E) -> (StatusCode, Json<ApiError>) {
    ...
    (
        ...
        Json(ApiError { error: "local storage operation failed".into() }),
    )
}
